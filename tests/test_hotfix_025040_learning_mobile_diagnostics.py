...
def test_release_version_is_025040():
 assert json.loads((COMP/'manifest.json').read_text())['version']=='0.25.0.47'; assert 'VERSION = "0.25.0.47"' in (COMP/'const.py').read_text()
...
def test_android_native_scroll_and_global_viewport_restore():
 t=(COMP/'frontend'/'freshairiq-card.js').read_text(); assert 'touch-action:pan-y pinch-zoom' in t and 'overflow-y:auto' in t; assert 'addEventListener("touchmove"' not in t; assert '_captureOverlayViewport()' in t and '_restoreOverlayViewport()' in t and '_closeOverlayToCard()' in t
