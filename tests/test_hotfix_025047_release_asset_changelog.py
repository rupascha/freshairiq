"""Regression contracts for v0.25.0.56 release-asset/changelog hygiene."""
...
def test_release_metadata_is_025047_and_hygiene_named():
    ...
    assert policy["version"] == "0.25.0.56"
    assert package["version"] == "0.25.0.56"
    assert str(policy["release"]["artifact_suffix"]).strip()
    assert (ROOT / "RELEASE_NOTES_0.25.0.56.md").is_file()
...
def test_mini_hotfix_does_not_touch_core_runtime_files_beyond_version_markers():
    ...
    assert 'VERSION = "0.25.0.56"' in const
    assert '"version": "0.25.0.56"' in manifest
