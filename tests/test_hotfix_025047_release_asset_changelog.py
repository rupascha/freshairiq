"""Regression contracts for v0.25.0.55 release-asset/changelog hygiene."""
...
def test_release_metadata_is_025047_and_hygiene_named():
    ...
    assert policy["version"] == "0.25.0.55"
    assert package["version"] == "0.25.0.55"
    assert str(policy["release"]["artifact_suffix"]).strip()
    assert (ROOT / "RELEASE_NOTES_0.25.0.55.md").is_file()
