"""Regression contracts for v0.25.1.8 release-asset/changelog hygiene."""
...
def test_release_metadata_is_025047_and_hygiene_named():
    ...
    assert manifest["version"] == "0.25.1.8"
    assert policy["version"] == "0.25.1.8"
    assert package["version"] == "0.25.1.8"
    assert str(policy["release"]["artifact_suffix"]).strip()
    assert (ROOT / "docs" / "releases" / "RELEASE_NOTES_0.25.1.8.md").is_file()
...
def test_mini_hotfix_does_not_touch_core_runtime_files_beyond_version_markers():
    ...
    assert 'VERSION = "0.25.1.8"' in const
    assert '"version": "0.25.1.8"' in manifest
