"""Regression contracts for v0.25.1.6 maximum hardening hotfix."""
...
def test_staging_copy_is_version_neutral_and_release_versions_match() -> None:
    ...
    policy = json.loads((ROOT / "quality" / "quality_policy.json").read_text(encoding="utf-8"))
    assert manifest["version"] == "0.25.1.6"
    assert policy["version"] == "0.25.1.6"
    assert 'VERSION = "0.25.1.6"' in (COMP / "const.py").read_text(encoding="utf-8")
    for name in ("freshairiq-card.js", "freshairiq-panel.js", "freshairiq-loader.js"):
        assert 'const FAIQ_VERSION = "0.25.1.6";' in (COMP / "frontend" / name).read_text(encoding="utf-8")
