"""Regression contracts for v0.25.0.66 partial forecast validation hotfix."""
...
def test_release_version_025036():
    assert 'VERSION = "0.25.0.66"' in (COMP / "const.py").read_text()
    assert '"version": "0.25.0.66"' in (COMP / "manifest.json").read_text()
    for name in ("freshairiq-card.js", "freshairiq-panel.js", "freshairiq-loader.js"):
        assert 'const FAIQ_VERSION = "0.25.0.66";' in (COMP / "frontend" / name).read_text()
