...
def test_lovelace_dependency_and_version():
    ...
    assert manifest["version"] == "0.25.0.43"
...
def test_all_version_markers_match():
    ...
    assert 'const FAIQ_VERSION = "0.25.0.43";' in (COMP / "frontend/freshairiq-card.js").read_text(encoding="utf-8")
    assert "Current release: 0.25.0.43" in (ROOT / "README.md").read_text(encoding="utf-8")
