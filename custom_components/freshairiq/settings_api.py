...
# The dashboard and Devices & Services use one canonical option contract.
# NATIVE_OPTION_KEYS is declared next to the native options flow, so adding or
# removing a native preference cannot silently create a second dashboard-only
# or native-only setting.
EDITABLE_OPTION_KEYS = set(NATIVE_OPTION_KEYS)
...
def _entry_payload(hass: HomeAssistant, entry: FreshAirIQConfigEntry) -> dict[str, Any]:
    ...
    return {
        ...
        "settings_contract": {
            "version": 1,
            "option_keys": sorted(EDITABLE_OPTION_KEYS),
