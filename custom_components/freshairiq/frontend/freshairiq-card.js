      ...
      <div class="iq-process"><div class="iq-process-head"><span class="iq-pulse"></span><b>IQ AKTIV</b><span>Abschlussauswertung</span></div><div class="iq-process-text">Kein Stillstand: FreshAirIQ wartet bewusst kurz auf eine aktuelle Sensor-Rückmeldung. Antwortet ein Batteriesensor nicht, endet die Gnadenfrist automatisch und die Messqualität wird aus den tatsächlich während der Lüftung beobachteten Meldungen bewertet.</div></div>
        ...
        if (active.length) {
            ...
            if (showNight && nightStrategy.active && hasNightComparison && Math.abs(nightWith - nightWithout) >= 1) {
                ...
                contextCards.push(`<div class="decision-impact night-context clickable" data-info="night"><span>BIS MORGEN · OHNE VORLÜFTEN</span><b>${nightWithout >= 0 ? "+" : ""}${Math.round(nightWithout)} ml Feuchte</b><small>erwartete Veränderung ohne zusätzliche Maßnahme</small></div>`);
                contextCards.push(`<div class="decision-impact night-context clickable" data-info="night"><span>BIS MORGEN · MIT EMPFEHLUNG</span><b>${nightWith >= 0 ? "+" : ""}${Math.round(nightWith)} ml Feuchte</b><small>${nightBenefit > 0 ? `≈ ${nightBenefit} ml weniger am Morgen · ` : ""}${esc(nightStrategy.action === "open_selected" ? "kontrollierte Nachtlüftung" : nightStrategy.action === "pre_ventilate" ? "vor dem Schlafengehen kurz lüften, dann schließen" : "empfohlenen Fensterzustand nutzen")}</small></div>`);
            }
