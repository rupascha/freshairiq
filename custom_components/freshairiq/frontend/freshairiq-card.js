...
class FreshAirIQCard extends HTMLElement {
    ...
    constructor() { super(); this.attachShadow({ mode: "open" }); this._config = {}; this._hass = null; this._dialogOpen = false; this._info = null; this._infoStack = []; this._dialogScrollTop = 0; this._subdialogScrollTop = 0; this._forceDialogTop = false; this._postResultTimer = null; this._settingsData = null; this._settingsLoading = false; this._settingsSaving = false; this._settingsError = null; this._settingsNotice = null; this._renderFrame = null; this._renderFrameIsRaf = false; this._liveRefreshFrame = null; this._liveRefreshFrameIsRaf = false; this._statusEntityId = null; this._statusRescanNeeded = false; this._relevantStateIds = null; this._roomEntityIds = null; this._roomConfigSignature = null; this._entityCache = {}; this._profileOverride = null; this._forecastOverride = null; this._fieldTestRegistrationPromise = null; this._fieldTestSessionClientId = null; this._liveViewSnapshot = null; this._liveHtmlCache = {}; this._chartCache = { bars: new WeakMap(), line: new WeakMap() }; this._learningCardCache = null; }
    _ensureStaticStyle() {
      ...
      <div class="iq-process"><div class="iq-process-head"><span class="iq-pulse"></span><b>IQ AKTIV</b><span>Abschlussauswertung</span></div><div class="iq-process-text">Kein Stillstand: FreshAirIQ wartet bewusst kurz auf eine aktuelle Sensor-Rückmeldung. Antwortet ein Batteriesensor nicht, endet die Gnadenfrist automatisch und die Messqualität wird aus den tatsächlich während der Lüftung beobachteten Meldungen bewertet.</div></div>
        ...
        if (active.length) {
            ...
            if (showNight && nightStrategy.active && hasNightComparison && Math.abs(nightWith - nightWithout) >= 1) {
                contextCards.push(`<div class="decision-impact night-context clickable" data-info="night"><span>NACHT · OHNE STRATEGIE</span><b>${nightWithout >= 0 ? "+" : ""}${Math.round(nightWithout)} ml</b><small>bis Nachtende</small></div>`);
                contextCards.push(`<div class="decision-impact night-context clickable" data-info="night"><span>NACHT · MIT STRATEGIE</span><b>${nightWith >= 0 ? "+" : ""}${Math.round(nightWith)} ml</b><small>${esc(nightStrategy.action === "open_selected" ? "kontrollierte Nachtlüftung" : nightStrategy.action === "pre_ventilate" ? "Vorlüften + schließen" : "empfohlener Fensterzustand")}</small></div>`);
            }
        ...
        if (showNight && nightStrategy.active && !nightPrimary) {
            ...
            const nightAction = nightStrategy.action === "close" ? "geschlossen" : nightStrategy.action === "open_selected" ? (nightRooms.length ? `${nightRooms.join(" + ")} teilweise offen` : "teilweise offen") : nightStrategy.action === "pre_ventilate" ? (nightRooms.length ? `${nightRooms.join(" + ")} vorher lüften` : "vorher lüften") : "beobachten";
        ...
        const processText = passiveOpenMonitor
            ...
            : bad.length
                ...
                : showNight && nightStrategy.active
                    ? `Nachtmodell aktiv · Wetter, Außenfeuchte, Temperatur und Fensterzustände werden bis zum Morgen abgeglichen`
    ...
    _infoPanel(st, rooms) {
        ...
        if (this._info === "mould") {
            ...
            return `<section class="info-panel"><div class="tiny info-kicker">SCHIMMEL-IQ · RISIKO NACH RÄUMEN</div><h3>Wo FreshAirIQ genauer hinschaut</h3><p>Die Räume sind nach Risiko sortiert. Die Schätzung kombiniert Raumklima und berechnete Oberflächenfeuchte. Tippe einen Raum an, um die Ursache, Empfehlung und Lernwerte zu sehen.</p><div class="breakdown">${rows}</div></section>`;
        }
        if (this._info === "rooms") {
    ...
    _details(st, rooms) {
        ...
        return `<div class="modal" role="dialog" aria-modal="true"><div class="dialog"><div class="dialog-head"><button id="details-back" class="dialog-back" aria-label="Zurück"><ha-icon icon="mdi:arrow-left"></ha-icon></button><div class="dialog-head-copy"><div class="tiny">FRESHAIRIQ DETAILS</div><div class="dialog-title">Hausklima & Lüftungsintelligenz</div></div><button id="settings-gear" class="settings-gear" aria-label="FreshAirIQ Einstellungen"><ha-icon icon="mdi:cog-outline"></ha-icon></button><button id="close" class="close" aria-label="Schließen">✕</button></div><div class="dialog-scroll"><div class="overview"><div class="summary clickable" data-info="water"><div class="tiny">WASSER IN DER LUFT</div><strong>${Math.round(Number(st.total_water_ml || 0))} ml</strong><span>über berechnete Räume</span></div><div class="summary clickable" data-info="threshold"><div class="tiny">LÜFTUNGSSCHWELLE</div><strong>${Math.round(Number(st.ventilation_threshold_ml || 0))} ml</strong><span>${st.ventilation_threshold_mode === "adaptive_home_size" ? "adaptiv · Ziel ca. 3–5×/Tag" : `${fmt(st.ventilation_threshold_percent || 10, 1)} % der Wassermenge`}</span></div><div class="summary clickable" data-info="night"><div class="tiny">NACHTPROGNOSE</div><strong style="color:#ff9b7a">+${Math.round(Number(st.overnight_forecast_ml || 0))} ml</strong><span>${Number(st.night_model_samples || 0)} Lernproben</span></div><div class="summary clickable" data-info="pollen"><div class="tiny">POLLEN</div><strong style="color:${st.pollen_blocked ? "#ff7770" : "#67df92"}">${st.pollen_enabled ? fmt(st.pollen_index, 1) : "aus"}</strong><span>${st.pollen_enabled ? `Grenze ${fmt(st.pollen_limit, 1)}` : "nicht berücksichtigt"}</span></div></div><div class="history-grid"><div class="history"><div class="history-head"><div><div class="tiny">LETZTE ${days} TAGE</div><b>Wasser in der Hausluft · Tagesmittel</b></div><strong>${fmt(Number(((_a = lastItem((st.water_history_14d || []).filter(x => x.samples > 0))) === null || _a === void 0 ? void 0 : _a.water_ml) || 0) / 1000, 2)} l</strong></div><div class="chart">${this._svgBars(st.water_history_14d || [])}</div><div class="muted">Absolute Feuchte × Raumvolumen, über alle überwachten Räume aggregiert. Tageswert = Mittel aller gültigen Messungen; Trend: ${esc(((_b = lastItem((st.water_history_14d || []).filter(x => x.samples > 0))) === null || _b === void 0 ? void 0 : _b.trend) || "stabil")}.</div></div><div class="history"><div class="history-head"><div><div class="tiny">ANWESENHEIT</div><b>Presence Intelligence</b></div><strong>IQ ${Math.round(Number(st.presence_confidence || 0))}%</strong></div><div class="muted">${esc(st.presence_explanation || "")} ${st.pets_in_household ? "Haustiermodus aktiv; klassische Bewegung wird schwächer gewichtet." : ""}</div><div class="muted">${Number(st.adult_occupants || 0)} Erwachsene · ${Number(st.child_occupants || 0)} Kinder · aktuell ${fmt(st.effective_occupants || 0, 1)} zuhause/erwartet · Gäste ${Number(st.guest_adults || 0)}+${Number(st.guest_children || 0)} · ${esc(propertyDE(st.property_type))} · ${esc(heatingDE(st.heating_system))}</div></div></div>${(() => { var _a, _b, _c, _d, _e, _f; const sim = (((_b = (_a = st.iq_state) === null || _a === void 0 ? void 0 : _a.decision) === null || _b === void 0 ? void 0 : _b.simulated_options) || []).slice(0, 5); if (!sim.length)
            return ""; const best = (_d = (_c = st.iq_state) === null || _c === void 0 ? void 0 : _c.decision) === null || _d === void 0 ? void 0 : _d.selected_option_id; return `<div class="history" style="margin-top:8px"><div class="history-head"><div><div class="tiny">IQ ENTSCHEIDUNGSSIMULATION</div><b>${esc(((_f = (_e = st.iq_state) === null || _e === void 0 ? void 0 : _e.decision) === null || _f === void 0 ? void 0 : _f.selected_option_label) || "Beste Option")}</b></div><strong>${sim.length} Optionen</strong></div><div class="breakdown">${sim.map(x => `<div class="breakdown-row"><b>${x.id === best ? "✓ " : ""}${esc(x.label || x.id)}</b><span>${x.duration_min ? `${fmt(x.duration_min, 1)} min · ` : ""}${Number(x.removed_ml || 0) > 0 ? `−${Math.round(Number(x.removed_ml))} ml · ` : ""}${signed(x.temperature_change_c || 0, "°C")} · ${fmt(x.cost || 0, 2)} € · IQ ${Math.round(Number(x.confidence || 0))}%${x.future_weather ? " · Wetterprognose" : ""}${Number(x.strategy_maturity || 0) >= 12 ? ` · Umsetzung ${fmt(x.strategy_follow_probability || 0, 0)} %` : ""}</span><strong>${x.id === best ? "gewählt" : "Alternative"}</strong></div>`).join("")}</div><div class="muted">FreshAirIQ zeigt hier nur verständliche Größen wie Feuchtewirkung, Temperatur, Kosten und Sicherheit. Interne Bewertungskennzahlen werden bewusst nicht angezeigt.</div></div>`; })()}${this._learningComponentsCard(st, rooms)}${lastCard}<div class="history" style="margin-top:8px"><div class="history-head"><div><div class="tiny">DIAGNOSE & TEST</div><b>FreshAirIQ Testaufzeichnung</b></div><strong style="color:${((_c = st.diagnostics) === null || _c === void 0 ? void 0 : _c.last_error) ? "#ff7770" : "#67df92"}">${((_d = st.diagnostics) === null || _d === void 0 ? void 0 : _d.last_error) ? "Fehler" : "Aktiv"}</strong></div><div class="muted">FreshAirIQ protokolliert anonymisierte Mess-, Prognose-, Entscheidungs- und Lerndaten rollierend für ${Number(((_e = st.diagnostics) === null || _e === void 0 ? void 0 : _e.retention_days) || 30)} Tage. Für Feldtests werden zusätzlich eine dauerhaft zufällige Installationskennung, Home-Assistant-/Systemversionen sowie anonymisierte Geräte-/Plattformdaten erfasst. Keine Home-Assistant Entity-IDs, Zugangsdaten, Koordinaten, Gerätenamen oder Seriennummern werden exportiert. Raumnamen bleiben für die Auswertung erhalten.</div><div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:10px"><span class="muted">${((_f = st.diagnostics) === null || _f === void 0 ? void 0 : _f.last_recorded_at) ? `Letzter Datensatz ${esc(whenDE(st.diagnostics.last_recorded_at))}` : "Aufzeichnung startet mit dem nächsten Datenzyklus"}</span><button class="quick-forecast" id="diagnostics-export"><ha-icon icon="mdi:download"></ha-icon><b>Diagnosedaten</b><span>exportieren</span></button></div>${((_g = st.diagnostics) === null || _g === void 0 ? void 0 : _g.last_error) ? `<div class="muted" style="color:#ff7770;margin-top:6px">${esc(st.diagnostics.last_error)}</div>` : ""}</div></div></div></div>`;
    }
    ...
    _render() {
        ...
        const oldSubScroll = this._subdialogScrollTop;
        ...
        (_m = this.shadowRoot.getElementById("details")) === null || _m === void 0 ? void 0 : _m.addEventListener("click", e => { e.stopPropagation(); this._dialogOpen = true; this._info = null; this._render(); });
        (_o = this.shadowRoot.getElementById("rooms")) === null || _o === void 0 ? void 0 : _o.addEventListener("click", e => { e.stopPropagation(); clearTimeout(this._deferredRender); this._deferredRender = null; this._infoStack = []; this._subdialogScrollTop = 0; this._info = "rooms"; this._render(); });
        (_p = this.shadowRoot.getElementById("guests")) === null || _p === void 0 ? void 0 : _p.addEventListener("click", e => { e.stopPropagation(); this._infoStack = []; this._subdialogScrollTop = 0; this._info = "guests"; this._render(); });
        (_q = this.shadowRoot.getElementById("close")) === null || _q === void 0 ? void 0 : _q.addEventListener("click", e => { e.stopPropagation(); this._dialogOpen = false; this._info = null; this._infoStack = []; this._render(); });
        const detailsBack = this.shadowRoot.getElementById("details-back");
        if (detailsBack) detailsBack.addEventListener("click", e => { e.stopPropagation(); this._dialogOpen = false; this._info = null; this._infoStack = []; this._render(); });
        ...
        if (feedbackSend) feedbackSend.addEventListener("click", async (e) => {
            ...
            try { const response=await this._hass.callApi("POST", `freshairiq/feedback/${entryId}`, {type,message}); if(response?.error) throw new Error(response.error); if(result) result.textContent=`Gesendet · Feedback-ID ${response.feedback_id||"erstellt"}`; const box=this.shadowRoot.getElementById("feedback-message"); if(box) box.value=""; } catch(err) { if(result) result.textContent=`Senden fehlgeschlagen: ${err?.message||err}`; } finally { feedbackSend.disabled=false; }
        });
        ...
        (_s = this.shadowRoot.getElementById("info-close")) === null || _s === void 0 ? void 0 : _s.addEventListener("click", e => { e.stopPropagation(); this._info = null; this._infoStack = []; this._subdialogScrollTop = 0; this._render(); });
        (_t = this.shadowRoot.getElementById("info-back")) === null || _t === void 0 ? void 0 : _t.addEventListener("click", e => { e.stopPropagation(); clearTimeout(this._deferredRender); this._deferredRender = null; this._subdialogScrollTop = 0; this._info = this._infoStack.length ? this._infoStack.pop() : null; this._render(); });
        ...
        this.shadowRoot.querySelectorAll("[data-info]").forEach(el => el.addEventListener("click", async e => { e.stopPropagation(); const next = el.dataset.info; if (!next || next === this._info) return; if (this._info) this._infoStack.push(this._info); else this._infoStack = []; this._subdialogScrollTop = 0; this._info = next; this._render(); if (next === "threshold" && !this._settingsData) await this._loadSettings(true); }));
        this.shadowRoot.querySelectorAll("[data-room]").forEach(el => el.addEventListener("click", e => { e.stopPropagation(); clearTimeout(this._deferredRender); this._deferredRender = null; const next = `room:${el.dataset.room}`; if (this._info && this._info !== next) this._infoStack.push(this._info); else if (!this._info) this._infoStack = []; this._subdialogScrollTop = 0; this._info = next; this._render(); }));
        ...
        if (newSubdialog) {
            ...
            let touchY = null;
            newSubdialog.addEventListener("touchstart", e => {
                var _a, _b, _c;
                touchY = (_c = (_b = (_a = e.touches) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.clientY) !== null && _c !== void 0 ? _c : null;
            }, { passive: true });
            newSubdialog.addEventListener("touchmove", e => {
                var _a, _b;
                const y = (_b = (_a = e.touches) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.clientY;
                if (touchY == null || y == null)
                    return;
                const delta = y - touchY;
                const atTop = newSubdialog.scrollTop <= 0;
                const atBottom = Math.ceil(newSubdialog.scrollTop + newSubdialog.clientHeight) >= newSubdialog.scrollHeight;
                if ((atTop && delta > 0) || (atBottom && delta < 0))
                    e.preventDefault();
                e.stopPropagation();
                touchY = y;
            }, { passive: false });
            newSubdialog.addEventListener("touchend", () => { touchY = null; }, { passive: true });
            newSubdialog.addEventListener("touchcancel", () => { touchY = null; }, { passive: true });
